...
const SkillDataProvider = memo(({ src, name, width, height, index, category }: SkillDataProviderProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        <p className="text-center font-medium text-white text-xs leading-tight">{name}</p>
      </div>
    </div>
  );
});
...
const mapToNewCategory = (oldCategory: string): string => {
  switch (oldCategory) {
    case 'Programming L & Tools': return 'Programming Languages';
    case 'Frameworks & Libraries': return 'Frontend';
    case 'Databases': return 'Databases & Cloud';
    case 'Design': return 'Tools & DevOps';
    default: return oldCategory;
  }
};
...
const categorizeSkill = (skillName: string): string => {
  ...
  if (name.includes('html') || name.includes('css') || name.includes('javascript') || 
      ...
      name.includes('c++') || name === 'c') return 'Programming Languages';
  if (name.includes('react') || name.includes('next') || name.includes('tailwind') || 
      name.includes('redux') || name.includes('framer') || name.includes('vite')) return 'Frontend';
  if (name.includes('node') || name.includes('express') || name.includes('graphql')) return 'Backend';
...
export const Skills = () => {
  ...
  useEffect(() => {
    const fetchSkillsData = async () => {
      ...
      const categorizedSkills = (data as Skill[]).map(skill => ({
        ...skill,
        category: skill.category ? mapToNewCategory(skill.category) : categorizeSkill(skill.skill_name)
      }));
  ...
  // Performance: Use useMemo for filtering instead of re-calculating on every render
  const filteredSkills = useMemo(() => 
    skills.filter(skill => activeCategory === 'All Skills' || skill.category === activeCategory),
        ...
        <div className="w-full max-w-2xl mb-12">
          <div className={`flex p-1 rounded-lg bg-white/5 border ${currentProps.border} transition-colors duration-300`}>
            {CATEGORIES.map((category) => {
              ...
              const skillCount = category === 'All Skills' ? skills.length : skills.filter(s => s.category === category).length;
              return (
                <button
                  key={category}
                ...
                >
                  {category} <span className="ml-1 text-xs opacity-70">({skillCount})</span>
                </button>
              );
            })}
          </div>
